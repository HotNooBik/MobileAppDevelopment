...
import com.pw2.stolbokapp.data.calendar.WeatherForecastCalendarService
import com.pw2.stolbokapp.data.calendar.toDomain
import com.pw2.stolbokapp.data.calendar.toEntity
...
class CalendarFragment : Fragment() {
    ...
    private val calendarDaysService: CalendarDaysService = WeatherForecastCalendarService()
    ...
    private fun observeCalendarDays(view: View) {
        viewLifecycleOwner.lifecycleScope.launch {
            db.calendarDayDao().getAllCalendarDays().collect { entities ->
                calendarDays = entities.map { it.toDomain() }.sortedBy { it.date }
                ...
                selectedYearMonth = pendingTargetDate?.let { YearMonth.of(it.year, it.monthValue) }
                    ...
                    ?: selectedYearMonth?.takeIf { existing ->
                        availableMonthsByValue.values.any { it == existing }
                    }
    ...
    private fun refreshCalendarData() {
        viewLifecycleOwner.lifecycleScope.launch {
            try {
                val days = calendarDaysService.getCalendarDays()
                if (days.isNotEmpty()) {
                    db.calendarDayDao().replaceAll(days.map { it.toEntity() })
                }
            } catch (e: Exception) {
                e.printStackTrace()
                val message = if (calendarDays.isEmpty()) {
                    "Не удалось загрузить календарь"
                } else {
                    "Не удалось обновить календарь, показаны сохраненные данные"
                }
    ...
    private fun setupCurrentDayInfo(view: View) {
        val todayData = currentCalendarDay
        if (todayData == null) {
            view.findViewById<TextView>(R.id.tvCurrentMonth).text = "..."
            view.findViewById<TextView>(R.id.tvCurrentDayNumber).apply {
                text = "..."
                background = ContextCompat.getDrawable(requireContext(), R.drawable.bg_day_none)
            }
            view.findViewById<TextView>(R.id.tvCurrentDayStatus).text = "Нет данных"
            view.findViewById<TextView>(R.id.tvCurrentWeather).apply {
                text = ""
                setCompoundDrawablesRelative(null, null, null, null)
            }
            view.findViewById<View>(R.id.constraintLayout).setOnClickListener(null)
            return
        }
